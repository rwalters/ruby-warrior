...
describe RubyWarrior::Profile do
  ...
  context "clearing epic scores and resetting last level number" do
    before do
    ...
    it { expect(@profile).to_not be_epic }
    it { expect(@profile.epic_score).to be_zero }
    it { expect(@profile.current_epic_score).to be_zero }
    it { expect(@profile.last_level_number).to be_nil }
    it { expect(@profile.average_grade).to be_nil }
    it { expect(@profile.current_epic_grades).to be_empty }
    it { expect(@profile.level_number).to eq 7 }
  end
